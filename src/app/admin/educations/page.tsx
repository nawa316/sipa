  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(item =>
        (item.institution && item.institution.toString().toLowerCase().includes(searchTerm.toLowerCase())) ||
        (item.degree && item.degree.toString().toLowerCase().includes(searchTerm.toLowerCase()))
      );
    }
    ...
    <div>
      ...
      <div className="p-8">
        <div className="mb-6 flex flex-col md:flex-row gap-4 justify-between">
          ...
          <button
            onClick={() => router.push('/admin/educations/create')}
              ...
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Degree & Major</th>
                  ...
                  {filteredData.map((item) => (
                    <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right text-sm font-medium">
                        <button
                          onClick={() => router.push(`/admin/educations/${item.id}`)}
                        ...
                        <button
                          onClick={() => setDeleteModal({ isOpen: true, id: item.id, title: item.institution || 'Item' })}
